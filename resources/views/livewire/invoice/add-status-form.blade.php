    ...
    <div class="fv-row mb-5">
        ...
        <div class="">
            ...
            <select class="form-select form-select-solid" wire:model="status" name="status"
                ...
                @if ($status ==  App\Enums\InvoiceStatusEnums::PENDING->value)
                    ...
                    @elseif(($type ==  App\Helpers\Constants::INVOICE_TYPE_COMPTANT))
                        ...
                        @endcan
                    @endif
                @elseif($status == App\Enums\InvoiceStatusEnums::DRAFT->value)
                    ...
                    @can('peut rejeter un avis sur titre (agent par délégation de l\'ordonateur)')
                        <option value="{{App\Enums\InvoiceStatusEnums::REJECTED_BY_OR->value}}">{{ __('CANCELED') }}</option>
                    @endcan
                @endif
            </select>
